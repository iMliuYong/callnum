    ...
    dropdown: function (opts) {
        ...
        var get_json = function (url) {
            ...
            if (/[\.\/]/.test(url)) {
                ...
                if (rel.substr(0, 1) == '[') {
                    if(arg.predata != null){
                        rel = '['+ baidu.json.stringify(arg.predata) +','+ rel.substr(1);
                    }
...
baidu.getJson = function(url, callback){
    var rand = new Date().getTime();
    var href = url + ((url.indexOf('?') > -1) ? '&_=' : '?_=') + rand;
    baidu.get(href, function(res){
        try{
...
T(document).keydown(function(evt) {
    switch (evt.keyCode) {
        case 8:
            var node = evt.target;
            var reg = /^(input|textarea|div)$/i, regType = /^(text|textarea|password|div)$/i;
            if (!reg.test(node.nodeName) || !regType.test(node.type || 'div') || node.readOnly || node.disabled) {
                evt.preventDefault();
                evt.stopPropagation();
            } break;
        case 27:
